...
const SearchBar: React.FC = () => {
    ...
    return (
        <div className={"lg:max-w-2xl mx-auto"}>
            <div
                className={`border dark:border-gray-400 border-gray-300 flex mt-5 p-3 hover:shadow cursor-pointer ${keywordValue.length > maxChars ? 'rounded-t-2xl border-b-white' : 'rounded-full'}`}>
                <div className="mr-2">
            ...
            {suggestions.length > 0 && keywordValue.length > maxChars && (
                <div
                    ...
                    {suggestions.map((suggestion) => (
                        <div
                            key={suggestion.id}
                            className="p-2 hover:bg-gray-200 dark:hover:bg-gray-600 cursor-pointer flex justify-between items-center"
                        ...
                        >
                            ...
                            {suggestion.thumbnail ?
                                <img className={"w-8 h-8 object-fit rounded-full"} src={suggestion.thumbnail.lqip} alt={suggestion.thumbnail.alt_text}/> : null
                            }
                        </div>
                    ))}
                    <div className={"my-4"}>
